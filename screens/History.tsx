...
const History: React.FC = () => {
  ...
  useEffect(() => {
    if (user) {
      setActivities(mockDb.getActivity().filter(a => a.userId === user.uid));
      setWithdrawals(mockDb.getRequests().filter(w => w.userId === user.uid));
    }
  }, [user]);
              ...
              <div key={req.id} className="glass p-5 rounded-[24px] border border-white/5 relative overflow-hidden">
                ...
                {req.status === RequestStatus.PENDING && (
                  <div className="mt-4 pt-4 border-t border-white/5">
                    ...
                    <p className="text-[10px] text-gray-500 italic">Estimated arrival: within 24 hours</p>
                  </div>
                )}
                ...
                {req.status === RequestStatus.REJECTED && (
                   <div className="mt-4 p-4 glass rounded-xl border border-red-500/20 bg-red-500/5 flex gap-3">
                      ...
                      <p className="text-xs text-gray-400 leading-relaxed">
                        Request denied due to unusual account activity. Points have been returned.
                      </p>
                   </div>
                )}
              </div>
